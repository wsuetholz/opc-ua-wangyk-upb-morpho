 ...
 * Returns an array of payloads whose content is partially filled.
 * Chunk size, padding and payload is written, but footer and header is missing.
...
public class MessageToChunks implements Callable<ByteBuffer[]> 
{
	...
	{		
	  try {
		...
		bq.setChunkSize(chunkFactory.maxPayloadSize);
		...
		ByteBuffer[] payloads = bq.getChunks(len);
		
		if (payloads.length>ctx.maxRecvChunkCount && ctx.maxRecvChunkCount!=0)
			throw new ServiceResultException(StatusCodes.Bad_TcpMessageTooLarge);
