...
public class AsyncServerSocket extends AbstractState<ServerSocketState, IOException> {
	...
	ServerSocketAcceptable listener = new ServerSocketAcceptable() {
		...
		public void onConnectionAcceptable(ListenableServerSocketChannel socket) {
			try {
				...
				AsyncSocketImpl as = new AsyncSocketImpl(chan, executor, AsyncServerSocket.this.sel);
				fireConnected(as);
	...
	public AsyncServerSocket() throws IOException
	{
		this(
	 ...
	 * @return
	 */
	private static ThreadPoolExecutor createNewThreadPoolExecutor() {
		final ThreadPoolExecutor e = new ThreadPoolExecutor(
				1, 
				4,
		        5L, TimeUnit.SECONDS,
		        new LinkedBlockingQueue<Runnable>(),
		        StackUtils.newNamedThreadFactory("AsyncServerSocket"));
