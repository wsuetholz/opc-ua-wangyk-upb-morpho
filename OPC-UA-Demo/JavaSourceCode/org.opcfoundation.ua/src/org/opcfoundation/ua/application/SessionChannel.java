...
public class SessionChannel extends ChannelService implements RequestChannel {
	...
	public ActivateSessionResponse activate(UserIdentityToken identity, SignatureData identitySignature) throws ServiceResultException
	{
		...
		RSAPrivateKey signerKey = session.getClientPrivateKey().getPrivateKey();
		String algorithmUri = securityPolicy.getAsymmetricSignatureAlgorithmUri();
		byte[] dataToSign = session.getServerCertificate().getEncoded();
		if (session.getServerNonce() != null)
			dataToSign = ByteBufferUtils.concatenate(dataToSign , session.getServerNonce());
		SignatureData clientSignature;
		try {
			clientSignature = CertificateUtils.sign(signerKey, algorithmUri, dataToSign);
		...
		asreq.setLocaleIds( client.getLocaleIds() );
		asreq.setClientSoftwareCertificates(client.getSoftwareCertificates());
